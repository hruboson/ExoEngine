...
ExoGui::ExoGui(ExoWindow& window, ExoDevice& device, VkRenderPass renderPass, uint32_t imageCount, ExoDB db) : device{device}, window{window}, renderPass{renderPass}, imageCount{imageCount}, db{db} {
    ...
    io.Fonts->AddFontFromFileTTF("../external/dearimgui/Mulish-Regular.ttf", 20, NULL, ranges.Data); // path to engine and then external

    // Icons
    ImFontConfig config;
    config.MergeMode = true;
    static const ImWchar icon_ranges[] = {ICON_MIN_FA, ICON_MAX_FA, 0};
    io.Fonts->AddFontFromFileTTF("../external/dearimgui/fontawesome-webfont.ttf", 20, &config, icon_ranges);
...
void ExoGui::runGui(FrameInfo frameInfo) {
    ...
    }
    {
        ...
        ImGui::SliderInt(u8"Rychlost času", &timeSpeed, -1000, 1000);
        ImGui::SameLine();
    ...
    }
    {
        ...
        ImGui::Button(ICON_FA_LEVEL_DOWN);
        if (ImGui::IsItemActive()) {
            moveDir -= upDir;
        }
        ImGui::TableNextColumn();
