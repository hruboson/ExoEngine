...
namespace exo {
	...
	std::vector<char> ExoPipeline::readFile(const std::string& filepath) {
		std::ifstream file{ filepath, std::ios::ate | std::ios::binary }; // ate - start reading at the end of file, binary - read file as binary file
	...
	void ExoPipeline::createGraphicsPipeline(const std::string& vertFilepath, const std::string& fragFilepath, const PipelineConfigInfo& configInfo) {
		...
		*/
		...
		auto bindingDescriptions = ExoModel::Vertex::getBindingDescriptions();
		auto attributeDescriptions = ExoModel::Vertex::getAttributeDescriptions();
