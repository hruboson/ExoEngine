...
namespace exo {
	...
	void setEscape(GLFWwindow* window, int key, int scancode, int action, int mods) {
		if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS) {
			if (!KeyboardMouseMovementController::mainMenu) {
				KeyboardMouseMovementController::mainMenu = true;
			}
			else {
				KeyboardMouseMovementController::mainMenu = false;
			}
		}
	}

	KeyboardMouseMovementController::KeyboardMouseMovementController(GLFWwindow* window) {
		//KeyboardMouseMovementController::mainMenu = false;
		glfwSetKeyCallback(window, setEscape);
	}

	void KeyboardMouseMovementController::moveInPlaneXZ(GLFWwindow* window, float dt, ExoObject& object, double previous_window_x, double previous_window_y, float moveSpeed, float lookSpeed) {
		glm::vec3 rotate{ 0 };
